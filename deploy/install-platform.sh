...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  USER_REDIS_ADMIN_PASSWORD=$(head -c 24 /dev/urandom | base64 | tr -d '=\n' | tr '+/' '-_')
  cat > "$ENV_FILE" <<EOF
...
# Shared Redis with ACL-based per-project isolation.
#   * Admin URL: control-plane uses this to ACL SETUSER per-project users.
#   * Public host: what user pods see in REDIS_URL.
CP_USER_REDIS_ADMIN_PASSWORD=${USER_REDIS_ADMIN_PASSWORD}
CP_USER_REDIS_ADMIN_URL=redis://default:${USER_REDIS_ADMIN_PASSWORD}@redis:6379/0
CP_USER_REDIS_PUBLIC_HOST=${PLATFORM_IP}:6379

# Per-project S3 buckets on the platform's MinIO. Reuses CP_S3_* admin creds
# above to create buckets + IAM users + policies. Public host is what the
# pod sees in S3_ENDPOINT (different network view than the compose-internal
# admin connection).
CP_USER_S3_PUBLIC_HOST=${PLATFORM_IP}:9000
CP_USER_S3_PUBLIC_USE_SSL=false
